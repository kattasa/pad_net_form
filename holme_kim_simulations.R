...
pan_graphs_over_time <- mclapply(1:n_iter, function(iteration) { # simulate network n_iter times
  mclapply(1:20, function(m_nodes) { # iterate through all anchor number values
    ...
    if(!(paste0('./simulation_data/hk_networks/', m_nodes) %in% list.dirs('./simulation_data/hk_networks/', recursive = F))) {
      dir.create(paste0('./simulation_data/hk_networks/', m_nodes))
    }
    
    network_sim <- holme_kim_algo(m = m_nodes, n = n_nodes)
    write.table(x = network_sim, file = paste0('./simulation_data/hk_networks/',
