...
#         print(fn)
        ...
        n = len(x)
#         if np.mean(x) < 1.5 or np.mean(x) > np.sqrt(n):
        if np.mean(x) > np.sqrt(n):
        ...
        if len(np.unique(x)) == 1:
            ...
            if analysis.loc[fn, 'ppl']=='' or np.isnan(analysis.loc[fn, 'ppl']) or overwrite == True:
                analysis.loc[fn, 'n'] = n
            ...
            if analysis.loc[fn, 'dexp']=='' or np.isnan(analysis.loc[fn, 'dexp']) or overwrite == True:
    ...
    del(alpha,  xmin, ntail, L, ks, p, dexp, dlm, dstrexp, dplwc)
    return analysis
...
#     unique_datasets = np.unique(df.index)
    if permissive:
